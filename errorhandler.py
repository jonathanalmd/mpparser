...
def reportSyntaxError(value):
    if value == "(":
        ...
        if run_mode == "strips" and value.upper() == "PRECONDITIONS":
            ...
            if value[0] == "@":
                print("Semantic error in '%s' (definition in/before line %d)"%(value[1:],plex.lexer.lineno))
                ...
                print("Semantic error in ':predicates' definition (in/before line %d)"%(plex.lexer.lineno))
                print("\tPredicate name(s) repetition")
            elif value[0] == ">":
                print("Semantic error in action definition")
                print("\tAction '%s' is duplicated"%(value[1:]))
            elif value[:2] == "t*":
                print("Semantic error in type '%s' definition (in/before line %d)"%(value[2:],plex.lexer.lineno))
                print("\tType name(s) redefinition") 
            elif value[:2] == "c*":
                print("Semantic error in ':constants' definition (in/before line %d)"%(plex.lexer.lineno))
                print("\tConstant name(s) repetition")
            elif value[:2] == "f*":
                print("Semantic error in ':functions' definition (in/before line %d)"%(plex.lexer.lineno))
                print("\tFunction(s) redefinition")
            elif value[:3] == "vp*":
                print("Semantic error in var '%s' definition (in/before line %d)"%(value[3:],plex.lexer.lineno))
                print("\tVar name(s) redefinition") 

            elif value.upper() == "CONSTANTS":
                print("Semantic error in '%s' line %d"%(value,plex.lexer.lineno))
            ...
            if value[0] == "?":
                ...
                print("Semantic error in '%s' (definition in/before line %d)"%(value[2:],plex.lexer.lineno))
                print("\tObject '%s' redefined'"%(value[1:]))
            elif value[:3] == "ot*":
                print("Semantic error in ':objects' definition (definition in/before line %d)"%(plex.lexer.lineno))
                print("\tObjects from different types with the same name")

            elif value[0][0] == "]":
                print("Semantic error in problem domain definition in line %d"%(plex.lexer.lineno))
